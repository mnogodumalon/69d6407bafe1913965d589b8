    ...
    if os.path.exists(image_file):
        try:
            ...
            if m:
                ...
                print(f"[LILO-PREVIEW] Bild geladen: {m.group(1)}, {len(m.group(2))} chars base64")
        except Exception as e:
            print(f"[LILO-PREVIEW] Fehler beim Lesen der Bild-Datei: {e}")
...
- Wenn 'npm run build' erfolgreich ist, bist du fertig"""
    ...
    async with ClaudeSDKClient(options=options) as client:
        ...
        if user_image_data:
            ...
            print(f"[LILO-PREVIEW] Multimodaler Prompt gesendet (Text + Bild)")
        else:
