...
interface UnternehmensprofilDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Unternehmensprofil['fields']) => Promise<void>;
  defaultValues?: Unternehmensprofil['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function UnternehmensprofilDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: UnternehmensprofilDialogProps) {
  const [fields, setFields] = useState<Partial<Unternehmensprofil['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'unternehmensprofil');
      await onSubmit(clean as Unternehmensprofil['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "unternehmensname": string | null, // Unternehmensname\n  "rechtsform": LookupValue | null, // Rechtsform (select one key: "einzelunternehmen" | "gbr" | "ohg" | "kg" | "gmbh" | "ug" | "gmbh_co_kg" | "ag" | "sonstige") mapping: einzelunternehmen=Einzelunternehmen, gbr=GbR, ohg=OHG, kg=KG, gmbh=GmbH, ug=UG (haftungsbeschränkt), gmbh_co_kg=GmbH & Co. KG, ag=AG, sonstige=Sonstige\n  "branche": LookupValue | null, // Branche (select one key: "bauhandwerk" | "elektrohandwerk" | "shk" | "tischler" | "maler" | "kfz" | "metallbau" | "dachdecker" | "galabau" | "sonstiges_handwerk" | "dienstleistung" | "handel" | "sonstige_branche") mapping: bauhandwerk=Bauhandwerk, elektrohandwerk=Elektrohandwerk, shk=Sanitär/Heizung/Klima, tischler=Tischler/Schreiner, maler=Maler/Lackierer, kfz=Kfz-Handwerk, metallbau=Metallbau, dachdecker=Dachdeckerei, galabau=Garten- und Landschaftsbau, sonstiges_handwerk=Sonstiges Handwerk, dienstleistung=Dienstleistung, handel=Handel, sonstige_branche=Sonstige\n  "geschaeftsjahr": number | null, // Geschäftsjahr (Jahr)\n  "geschaeftsjahr_beginn": string | null, // YYYY-MM-DD\n  "geschaeftsjahr_ende": string | null, // YYYY-MM-DD\n  "ansprechpartner_vorname": string | null, // Vorname Ansprechpartner\n  "ansprechpartner_nachname": string | null, // Nachname Ansprechpartner\n  "ansprechpartner_email": string | null, // E-Mail Ansprechpartner\n  "ansprechpartner_telefon": string | null, // Telefon Ansprechpartner\n  "bemerkungen": string | null, // Bemerkungen zum Unternehmen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Unternehmensprofil['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Unternehmensprofil bearbeiten' : 'Unternehmensprofil hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                <SelectItem value="bauhandwerk">Bauhandwerk</SelectItem>
                <SelectItem value="elektrohandwerk">Elektrohandwerk</SelectItem>
                <SelectItem value="shk">Sanitär/Heizung/Klima</SelectItem>
                <SelectItem value="tischler">Tischler/Schreiner</SelectItem>
                <SelectItem value="maler">Maler/Lackierer</SelectItem>
                <SelectItem value="kfz">Kfz-Handwerk</SelectItem>
                <SelectItem value="metallbau">Metallbau</SelectItem>
                <SelectItem value="dachdecker">Dachdeckerei</SelectItem>
                <SelectItem value="galabau">Garten- und Landschaftsbau</SelectItem>
                <SelectItem value="sonstiges_handwerk">Sonstiges Handwerk</SelectItem>
                <SelectItem value="dienstleistung">Dienstleistung</SelectItem>
                <SelectItem value="handel">Handel</SelectItem>
                <SelectItem value="sonstige_branche">Sonstige</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <Label htmlFor="geschaeftsjahr">Geschäftsjahr (Jahr)</Label>
            <Input
              id="geschaeftsjahr"
              type="number"
              value={fields.geschaeftsjahr ?? ''}
              onChange={e => setFields(f => ({ ...f, geschaeftsjahr: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="geschaeftsjahr_beginn">Beginn des Geschäftsjahres</Label>
            <Input
              id="geschaeftsjahr_beginn"
              type="date"
              value={fields.geschaeftsjahr_beginn ?? ''}
              onChange={e => setFields(f => ({ ...f, geschaeftsjahr_beginn: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="geschaeftsjahr_ende">Ende des Geschäftsjahres</Label>
            <Input
              id="geschaeftsjahr_ende"
              type="date"
              value={fields.geschaeftsjahr_ende ?? ''}
              onChange={e => setFields(f => ({ ...f, geschaeftsjahr_ende: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="bemerkungen">Bemerkungen zum Unternehmen</Label>
            <Textarea
              id="bemerkungen"
